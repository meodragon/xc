...
bool SkinningPipeline::init(RenderData& renderData, VkPipelineLayout& pipelineLayout, VkPipeline& pipeline, std::string vertexShaderFilename, std::string fragmentShaderFilename) {
  /* shader */
  ...
  if (vertexModule == VK_NULL_HANDLE || fragmentModule == VK_NULL_HANDLE) {
      printf("%s error: could not load shaders\n", __FUNCTION__);
  ...
  std::vector<VkVertexInputBindingDescription> vertexBindings = { { 0, static_cast<uint32_t>(sizeof(Vertex)), VK_VERTEX_INPUT_RATE_VERTEX } };

  VkVertexInputAttributeDescription positionAttribute{};
  positionAttribute.binding = 0;
  positionAttribute.location = 0;
  positionAttribute.format = VK_FORMAT_R32G32B32_SFLOAT;
  positionAttribute.offset = static_cast<uint32_t>(offsetof(Vertex, position));

  VkVertexInputAttributeDescription colorAttribute{};
  colorAttribute.binding = 0;
  colorAttribute.location = 1;
  colorAttribute.format = VK_FORMAT_R32G32B32A32_SFLOAT;
  colorAttribute.offset = static_cast<uint32_t>(offsetof(Vertex, color));

  VkVertexInputAttributeDescription normalAttribute{};
  normalAttribute.binding = 0;
  normalAttribute.location = 2;
  normalAttribute.format = VK_FORMAT_R32G32B32_SFLOAT;
  normalAttribute.offset = static_cast<uint32_t>(offsetof(Vertex, normal));

  VkVertexInputAttributeDescription uvAttribute{};
  uvAttribute.binding = 0;
  uvAttribute.location = 3;
  uvAttribute.format = VK_FORMAT_R32G32_SFLOAT;
  uvAttribute.offset = static_cast<uint32_t>(offsetof(Vertex, uv));
  ...
  jointsAttribute.offset = static_cast<uint32_t>(offsetof(Vertex, boneNumber));
  ...
  weightAttribute.offset = static_cast<uint32_t>(offsetof(Vertex, boneWeight));
  ...
  if (result != VK_SUCCESS) {
    printf("%s error: could not create rendering pipeline (error: %i)\n", __FUNCTION__, result);
    Shader::cleanup(renderData.rdVkbDevice.device, vertexModule);
    Shader::cleanup(renderData.rdVkbDevice.device, fragmentModule);
...
void SkinningPipeline::cleanup(RenderData &renderData, VkPipeline &pipeline) {
